...
export const searchAll = async (query, amount) => {
    try {
        const unsplashResults = await searchUnsplash(query, amount)
        const pixabayResults = await searchPixabay(query, amount)
...
export const searchUnsplash = async (query, amount) => {
    try {
        const res = await axios.get(`${UNSPLASH_URL}&query=${query}&per_page=5&page=${amount}`)
        const { data } = res
        console.log(data)
...
export const searchPixabay = async (query, amount) => {
    try {
        const res = await axios.get(`${PIXABAY_URL}&q=${query}&image_type=photo&${amount}`)
        const { data } = res
        // console.log(data)
